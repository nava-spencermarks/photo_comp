...
class RobustFaceComparator:
    ...
    def preprocess_image_variations(self, image_path):
        ...
        try:
            ...
            if max(brightened.size) > 1200:
                ...
                new_size = (int(brightened.width * ratio), int(brightened.height * ratio))
        ...
        try:
            smaller = pil_image.resize((pil_image.width // 2, pil_image.height // 2), Image.Resampling.LANCZOS)
        ...
        try:
            ...
            for (x, y, w, h) in faces:
                ...
                if 0.8 <= aspect_ratio <= 1.25:  # More strict proportions
                    good_faces.append((y, x + w, y + h, x))  # Convert to face_recognition format
            ...
            try:
                ...
                if face_locations:
                    ...
                    if encodings:
                        ...
                        return encodings, f"Found {len(encodings)} faces using HOG on {var_name}"
            ...
            try:
                ...
                if face_locations:
                    ...
                    if encodings:
                        ...
                        return encodings, f"Found {len(encodings)} faces using HOG 2x on {var_name}"
            except Exception as e:
            ...
            try:
                ...
                if face_locations:
                    ...
                    if encodings:
                        ...
                        return encodings, f"Found {len(encodings)} faces using CNN on {var_name}"
            except Exception as e:
            ...
            if opencv_faces:
                try:
                    ...
                    if encodings:
                        ...
                        return encodings, f"Found {len(encodings)} faces using OpenCV fallback on {var_name}"
                except Exception as e:
    ...
    def compare_faces(self, image1_path, image2_path):
        ...
        print(f"Comparing {os.path.basename(image1_path)} vs {os.path.basename(image2_path)}")
        print("=" * 60)
        ...
        confidence = max(0, (self.tolerance - best_distance) / self.tolerance * 100) if best_distance != float('inf') else 0
